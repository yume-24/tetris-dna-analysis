...
VAR_WEIGHT = 1  # encourage diversity in design scores
USE_REINFORCE = False ###     ***This runs... but I wouldn't use it (REINFORCE)

USE_SIMPLE_MLP = True  # set to False to use CNN

MLP_HIDDEN_STRIDER = 80
MLP_LAYERS = 4
# MLP_HIDDEN = 10
# MLP_LAYERS = 3
...
#See; "Deep Sets with per-layer global residual injection"
"""
2. Set Transformer (Lee et al., 2019)

Learns global context via attention (PMA), not just pooling.
Combines invariant and equivariant set processing.
You're doing a simplified, MLP-only analog of that.
Cite:

Lee et al., Set Transformer, ICML 2019
https://arxiv.org/abs/1810.00825
3. MLP-Mixer (Tolstikhin et al., 2021)

Applies per-token and token-mixing MLPs in alternation.
Requires fixed-length inputs.
Your architecture is inspired by this, but replaces mixing with pooled context.
Cite:

Tolstikhin et al., MLP-Mixer: An all
"""
