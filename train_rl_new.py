...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.fc2= nn.Linear(64, DNA_LEN*4)
...
def train_model(
    ...
    print(f"Created actor with {total_params:,} parameters")
    total_params = sum(p.numel() for p in critic.parameters())
    print(f"Created critic with {total_params:,} parameters")
    ...
    for ep in range(1, EPOCHS+1):
        ...
        n_samp = 0
        s_corr = s_var = s_a = s_c = real_reward = 0.0
        ...
        for i, (boards, mask, scores) in enumerate(dl_tr):
            ...
            reward = corr  
            #### And the variance bonus
            var_bonus = -design.var()
            reward += VAR_WEIGHT * var_bonus
        ...
        train_reward     = real_reward / n_samp
        ...
        print(f"Ep {ep}/{EPOCHS} temp={temp:.2f}  "
              ...
              f"design_var={design_var:.4f}  train_reward={train_reward:.4f}")
